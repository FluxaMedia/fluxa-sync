-- Apply the compatibility document, event cursor, and typed domain row in one
-- server-owned transaction. The edge function validates the JSON shape first;
-- this RPC is deliberately not exposed to client roles.
...
begin
    ...
    if p_entity_type = 'library' then
        ...
        if p_deleted or coalesce((p_payload ->> 'watched')::boolean, true) = false then
            ...
            on conflict (profile_id, content_id, content_type, season, episode) do update set
